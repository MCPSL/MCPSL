function MSAAuthCodeRequest {
    ...
    return "https://login.microsoftonline.com/consumers/oauth2/v2.0/authorize?client_id=$ClientID&response_type=code&redirect_uri=$EncodedRedirectURI&scope=XboxLive.signin%20offline_access"
}
...
function MSAAuthCodeRedeem {
    ...
    $EncodedRedirectURI = [System.Web.HttpUtility]::UrlEncode($RedirectURI)
    return Invoke-WebRequest 'https://login.microsoftonline.com/consumers/oauth2/v2.0/token' -Method 'POST' -Body "client_id=$ClientID&scope=XboxLive.signin%20offline_access&code=$AuthorizationCode&redirect_uri=$EncodedRedirectURI&grant_type=authorization_code" | ConvertFrom-Json
}
